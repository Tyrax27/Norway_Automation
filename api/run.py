...
def find_effective_dates_in_text(text_full):
    ...
    # "I kraft fra 2026-01-01" / "I kraft frå2026-01-01"
    ...
    # "I kraft fra2025-08-29, 2025-09-15"
    for m in re.finditer(
...
def parse_law_xml(xml_bytes):
    ...
    # -------------------------
    # 1) POSITIVE entry-into-force candidates only
    # -------------------------
    ...
    # For debugging, keep full list including last amendment dates
...
def write_block(svc, start_row_1based, rows):
    ...
    titles = [[r.get("title", "")] for r in rows]   # C
    dates  = [[r.get("date", "")] for r in rows]    # H
    urls   = [[r.get("url", "")] for r in rows]     # S
    tvals  = [[2] if r.get("type") == "reg" else [""] for r in rows]  # T
...
def scrape_and_write():
    ...
    log(f"[scrape] total laws parsed: {len(candidate_laws)}")

    # Keep only in_force + ambiguous
    kept_laws = [l for l in candidate_laws if l["status"] in ("in_force", "ambiguous")]
    log(f"[scrape] kept in-force+ambiguous laws: {len(kept_laws)}")
    ...
    log(f"[scrape] limiting to {len(kept_laws)} laws")

    # Build insertion rows (law then regs)
    output_rows = []
    ambiguous_positions = []
    ...
    log(f"[scrape] total rows to append: {len(output_rows)}")
...
def handler(request):
    ...
    Hit /api/run.py to execute.
    """
    try:
        result = scrape_and_write()
        return {
            "statusCode": 200,
            "body": json.dumps(result, ensure_ascii=False)
        }
    except Exception as e:
        # show useful error in logs + response
